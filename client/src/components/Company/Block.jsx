import React, { useState } from 'react'
import { BsFillPencilFill } from 'react-icons/bs'
import Modal from './Modal'

const Block = ({
	title,
	visible,
	note,
	option1,
	option2,
	option3,
	option4,
	option5,
	title1,
	title2,
	title3,
	title4,
	title5,
}) => {
	const [isOpen, setIsOpen] = useState(false)
	return (
		<>
			{isOpen == true && <Modal type={title} state={isOpen} />}
			<div className="w-full h-auto flex flex-col items-center bg-stone-900">
				{visible ? (
					<>
						<div className="w-full p-3 h-full">
							<h2 className="text-sm font-semibold">{title}</h2>
						</div>
						<div
							className={`w-full ${
								note == '' && 'flex justify-center items-center hover:text-blue-400 hover:bg-stone-700'
							} min-h-[45px] max-h-[150px] p-3 bg-stone-800 duration-300`}
							onClick={() => setIsOpen(prev => !prev)}>
							{note != '' ? (
								<div className="h-full">
									<textarea className="block w-full h-auto max-h-[120px] text-xs bg-transparent resize-none">
										{note}
									</textarea>
								</div>
							) : (
								<>
									<BsFillPencilFill className="mr-1 text-xs" />
									<button className="text-xs">Zamień</button>
								</>
							)}
						</div>
					</>
				) : (
					<>
						<div className="flex flex-col justify-center items-center p-3 text-sm">
							<h2 className=" mb-5 text-sm font-semibold">{title}</h2>
							<div className="flex">
								<div className="text-left">
									<h3>{title1}</h3>
									<h3>{title2}</h3>
									<h3>{title3}</h3>
									<h3>{title4}</h3>
									<h3>{title5}</h3>
								</div>
								<div className="w-[3px] mx-2 bg-stone-400 rounded-sm"></div>
								<div>
									<h3>{option1}</h3>
									<h3>{option2}</h3>
									<h3>{option3}</h3>
									<h3>{option4}</h3>
									<h3>{option5}</h3>
								</div>
							</div>
						</div>
					</>
				)}
			</div>
		</>
	)
}

export default Block
